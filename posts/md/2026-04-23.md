# 加拿大 Work Permit 申请时间线和一些经验记录 / Canadian Work Permit Timeline and Some Notes from Our Experience
...
因为申请长时间没有实质进展，我们后来联系了中介 / 律师，并开始考虑 mandamus。
...
收到 dismiss 之后，我们又开始准备第二波 mandamus。

这里我不提供法律建议，因为每个 case 的情况、时间点、证据、律师判断都不一样。但不管最后是否走 mandamus，一个清楚的 timeline 都非常重要。你需要能说清楚什么时候申请、当时 processing time 是多少、什么时候超过 processing time、发过哪些 webform、调过哪些档、联系过哪些机构、delay 对工作和生活造成了什么具体影响。

### 七、通过 NSIRA 投诉 / follow up CSIS
...
* 申请人的职位和预期 start date；
* delay 对 appointment / employment 的影响；
* 对研究、学生、funding、teaching 或 relocation 的影响；
* 学校希望申请能够尽快处理。
...
* 联系 MP office；
* 联系学校；
* 通过 NSIRA 相关渠道 follow up / 投诉 CSIS；
* 得知 CSIS 其实在 2025 年 3 月底已经完成；
* 联系中介 / 律师；
* 申请第一波 mandamus；
* 第一波 mandamus 被 dismiss 后准备第二波；
* 从 2025 年 11 月开始每周给 CBSA 负责人写邮件；
* 收到 CBSA 回复，但回复没有提供实质信息；
* 让学校写 support letter 并提交给 IRCC；
* 持续在每一个 update 或 lack of update 之后继续 webform。

### 十一、一些可能有用的经验

#### 1. 从一开始就维护一个清楚的 timeline
...
#### 7. Grant / funding timeline 也要尽早沟通
...
#### 8. Candidate 阶段最好和学校讨论 backup plan
...
如果 work permit delay，start date 能不能调整？
appointment letter 和合同怎么处理？
relocation support 怎么办？
学生第一年的 support 有没有 backup？
teaching assignment 是否需要 contingency plan？
grant / fellowship timeline 能不能调整？

这些问题不需要一开始就讲得很吓人，但学校最好知道这是一个可能性。

#### 9. Mandamus 要和律师具体讨论，但 timeline 一定要早整理
...
### 2. Realizing the application was in security screening
...
### 3. IRCC webforms: we kept sending them, but the information was limited
...
### 4. Access-to-information requests: mainly CBSA records, but hard to interpret
...
### 5. Contacting an MP and keeping the university updated
...
### 6. First mandamus application

Because the application had no substantive progress for a long time, we contacted our representative / lawyer and started considering mandamus.
...
After that, we started preparing a second mandamus application.

I am not giving legal advice here. Every case is different, and the timing and evidence matter. But regardless of whether one goes down the mandamus route, it is important to have a very clear timeline: when the application was submitted, what the processing time was, when it exceeded the posted processing time, what webforms were submitted, what records were requested, which offices were contacted, and what concrete impact the delay had.

### 7. Following up with CSIS through NSIRA
...
### 8. Writing weekly emails to CBSA
...
* submitted IRCC webforms repeatedly;
* requested CBSA records;
* contacted an MP’s office;
* kept the university updated;
* followed up / filed a complaint related to CSIS through NSIRA;
* learned that CSIS had actually completed its part in late March 2025;
* worked with our representative / lawyer;
* filed a first mandamus application;
* prepared for a second mandamus application after the first was dismissed;
* wrote weekly emails to CBSA starting in November 2025;
* received a CBSA response that did not provide substantive information;
* asked the university to write a support letter and submitted it to IRCC;
* continued following up whenever there was an update, or when there was no update for a long time.

### 11. Some practical takeaways

#### Keep a clear timeline from the beginning
...
#### Webforms may not give useful information, but keep a record
...
#### Records requests are useful, but often incomplete
...
#### CSIS completion does not mean the application is complete
...
#### University communication matters a lot
...
#### Student and funding arrangements may need backup plans
...
#### Communicate early about grant / funding timelines
...
#### Discuss backup plans during the candidate / hiring stage when relevant
...
#### Mandamus requires legal advice, but timeline preparation helps either way
